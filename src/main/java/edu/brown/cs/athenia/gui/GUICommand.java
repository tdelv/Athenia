...
public class GUICommand {
  ...
  /**
   * GET request handler for the sign-in page of Athenia. Prompts the user
   * to sign-in via the Google API.
   */
  ...
  /**
   * GET request handler which pulls the different languages the user has so
   * far logged in the app and displays the prompt to choose from the options,
   * if they exist.
   */
  ...
  /**
   * GET request handler which pulls the most recent activity of the
   * appropriate user and presents this information on the home page of Athenia.
   */
  public class HomePageHandler implements TemplateViewRoute {
    ...
    public ModelAndView handle(Request req, Response res) {
      ...
      //      1. pull the recent activity of the user from the database or user object
  ...
  /**
   * GET request handler which pulls all of the vocabulary information saved
   * by the user in the database and formats it to send to the front end to
   * display on the "Vocabulary" landing page.
   */
  ...
  public class UpdateVocabularyHandler implements Route {
    ...
    public String handle(Request req, Response res) {
      QueryParamsMap qm = req.queryMap();
      // TODO: pull out the information of the vocabulary module and act according to the following:
  ...
  /**
   * GET request handler for retrieving information pertaining to a specific
   * tag the user uses and wishes to view all modules categorized by that tag.
   * Pulls this information from the database and formats it to present to
   * the front end.
   */
  public class TagLandingPageHandler implements TemplateViewRoute {
    ...
    public ModelAndView handle(Request req, Response res) {
      QueryParamsMap qm = req.queryMap();
      // TODO: parse out the tag label the user wishes to view (tag ID, tag Name)
      Map<String, Object> variables =
              new ImmutableMap.Builder<String, Object>()
              .put("...", "...").build();
   ...
   * pages. Specifically used for updating tag information and not for
   * adding or removing tags from a module.
   */
  ...
  /**
   * POST request handler for updating a module's tag set, particularly
   * adding or removing a tag to or from the module. Called wherever a module's
   * tag can be edited -- including on the vocabulary, conjugation, and
   * FreeNotes landing, and more importantly, individual pages as well as
   * the tag landing and individual pages.
   */
  public class UpdateModuleTagHandler implements Route {
    @Override
    public String handle(Request req, Response res) {
      QueryParamsMap qm = req.queryMap();
      // TODO pull in the information of the module, the tag, and the operation wanted:
   ...
   * database and formats this information to send to the front end for
   * display on the user's "Conjugation" landing page.
   */
  ...
  /**
   * POST request handler for adding, updating, or deleting a conjugation
   * module within the database. Called whenever the user makes an edit to
   * any conjugation module which can be found on the following pages:
   * the conjugation landing and individual pages and the FreeNotes landing
  ...
  public class UpdateConjugationHandler implements Route {
    ...
    public String handle(Request req, Response res) {
      QueryParamsMap qm = req.queryMap();
      // TODO pull out the information of the conjugation module and act on it accordingly:
  ...
  /**
   * GET request handler for an individual FreeNotes page which the user
   * wishes to access. Pulls out all of the information on that page and
   * organizes it in a way for the front end to display to the user.
   */
  public class FreeNotesIndividualPageHandler implements TemplateViewRoute {
    @Override
    public ModelAndView handle(Request req, Response res) {
      QueryParamsMap qm = req.queryMap();
      // TODO: determine which free note the user wants to view and find in database
      Map<String, Object> variables =
              new ImmutableMap.Builder<String, Object>()
              .put("...", "...").build();
      ...
      //   > involves a lot of module storing (a cache in both front end and backend?)
      return new ModelAndView(variables, "...");
    }
  }
   ...
   * page. These modules include a vocabulary module, a conjugation module,
   * a text box, or a note. Generates this module in the backend, adds it
   * to the areas in the backend where appropriate and inserts the information
   * into the database. Sends to the front-end the information necessary
   * for the user to use that module.
   */
   ...
   * page. Searches through the updates made and updates all of the areas in
   * the backend. Called sporadically as an auto save feature.
   */
  public class UpdateFreeNotesHandler implements Route {
    ...
    public String handle(Request req, Response res) {
      ...
      //        1. add new freenotes page (title, tags, date created, date accessed, date edited, etc.)
