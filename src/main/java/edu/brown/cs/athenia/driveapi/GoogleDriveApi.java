...
public class GoogleDriveApi {
     ...
     * @throws GoogleDriveApiException when anything goes wrong with API connection.
     */
     ...
     * @throws GoogleDriveApiException when anything goes wrong with API connection.
     */
    public static String getUrl(String state, String redirectUri)
            throws GoogleDriveApiException {
        try {
     ...
     * @throws GoogleDriveApiException when anything goes wrong with API connection.
     */
    public static void createCredential(String userId, String code, String redirectUri)
            throws GoogleDriveApiException {
        try {
    ...
    public static java.io.File getDataBase(String userId)
        ...
        try {
            // Java file pointing to location in server memory
            java.io.File file = new java.io.File(
                    "src/main/resources/userData/" + userId + ".sqlite3");

            // Connection to Google Drive for user
            Drive service = getService(userId);

            // Write Google Drive file to local file if it exists
            ...
            // If everything succeeds, store in cache and return
    ...
    public static void setDataBase(String userId, java.io.File dataBase)
        ...
        try {
            ...
            // Delete file from Google Drive if already exists;
            // Tried to update it before to keep track of versions,
            // but Google Drive was having issues with it.
            File driveFile = getDriveFile(service);
            if (driveFile != null) {
                service.files().delete(driveFile.getId()).execute();
            }

            // Set up data for creating Google Drive file
    ...
    /**
     * Gets the Google Drive file that contains the user data, or returns null if none.
     *
     * @param service the Drive service through which the new Drive file is created.
     * @return the Google Drive file for the user data.
     * @throws IOException when something goes wrong getting the list of files.
     */
    private static File getDriveFile(Drive service)
            throws IOException {
        // Get all AppData files
        ...
        // If file wasn't found, return null
        return null;
    }
    ...
    private static Drive getService(String userId)
        ...
        // Grab the service for the user's credentials
